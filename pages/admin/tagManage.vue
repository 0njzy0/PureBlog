    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title primary-title>
          <span class="headline">标签信息</span>
        </v-card-title>
        <v-card-text>
          <v-form ref="form">
            <v-text-field v-model="formData.name" required :rules="nameRules" label="标签名" type="text" />
          </v-form>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
  ...
  methods: {
    ...
    async createData() {
      if (this.$refs.form.validate()) {
        const res = await this.$axios.$post(`/tags`, this.formData)
        if (res && res.success) {
          this.getList()
    ...
    async updateData() {
      ...
      if (this.$refs.form.validate()) {
        const res = await this.$axios.$put(`/tags/${id}`, this.formData)
        if (res && res.success) {
          this.getList()
          this.handleDialogClose()
        }
